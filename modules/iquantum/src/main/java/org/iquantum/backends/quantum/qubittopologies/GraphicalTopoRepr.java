...
public class GraphicalTopoRepr {
    ...
    public static void repr(QubitTopology qbt) {
        ...
        for (QubitTopology.Node node : qubits) {
            graph.addNode(String.valueOf(node.qubitIndex));
        }
        for (int i = 0; i < nodesSize; i++) {
            String bNodeIndex = String.valueOf(qbt.getQubits().get(i).qubitIndex);
            int neighborsSize = qbt.getQubits().get(i).neighbors.size();
            // graph.addNode(bNodeIndex);
            for (int j = 0; j < neighborsSize; j++) {
                String nNodeIndex = String.valueOf(qbt.getQubits().get(i).getNeighbors().get(j).qubitIndex);
